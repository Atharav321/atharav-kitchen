/**
 * ============================================================
 *  ATHARAV KITCHEN — firestoreService.js
 *
 *  Responsibility: ALL Firestore read/write operations
 *    - Single place for db.collection() calls
 *    - Retry logic, offline handling, error normalisation
 *    - DO NOT call db.collection() directly in feature modules
 *
 *  Pattern:
 *    firestoreService.getOrder(id)       → Promise<order>
 *    firestoreService.updateOrderStatus(id, status) → Promise<void>
 *    firestoreService.getMenuItems()     → Promise<item[]>
 *
 *  Adding new db operations:
 *    1. Add function here with JSDoc
 *    2. Call it from the feature module (auth.js, orders.js etc.)
 *    3. Never duplicate Firestore logic across modules
 * ============================================================
 */
/* ================================================
...
function saveOrderWithRetry(orderId, orderObj, attempt) {
  ...
  return firebase
    ...
    .catch(function (e) {
      console.warn(
        '[Atharav Kitchen] Order save attempt ' + attempt + ' failed:',
        e.code || e.message
      );
      if (attempt < 3) {
        return new Promise(function (res) {
          setTimeout(res, 800 * attempt);
        }).then(function () {
...
function saveFeedback(fbObj) {
  ...
  return firebase
    ...
    .catch(function (e) {
      console.warn('[Atharav Kitchen] Feedback save failed:', e.code || e.message);
      return fb;
    });
}
