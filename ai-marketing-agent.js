  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/generate-blog-draft' && request.method === 'POST') {
      try {
        ...
        const verified = await verifyFirebaseIdToken(body.idToken, env);
        if (!verified.ok || verified.email !== env.ADMIN_EMAIL) {
          return json({ error: 'Sirf admin chala sakta hai', debug: debugBlock(body, verified, env) }, 403, cors);
        }
        ...
        if (body.keyword) {
          ...
          if (!opportunities.length) {
            return json({ error: 'Abhi koi achhi SEO opportunity keyword nahi mili — kam Search Console data hai. Khud ek keyword daal ke try karo.' }, 200, cors);
          }
        ...
        const draftDoc = {
          ...
          seoContext: { impressions: opportunity.impressions, position: opportunity.position, ctr: opportunity.ctr },
          ...draft,
        };
  ...
  try {
    ...
    if (opportunities.length) {
      ...
      blogDraftDoc = {
        ...
        seoContext: { impressions: topOpportunity.impressions, position: topOpportunity.position, ctr: topOpportunity.ctr },
        ...draft,
      };
