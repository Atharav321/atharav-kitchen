  ...
  async fetch(request, env) {
    ...
    if (!env.AI) {
      return json(
        {
          error:
            'Workers AI binding missing. Worker Settings → Bindings mein "AI" binding add karo (see file comment).',
        },
        500,
        cors
      );
    ...
    if (!idToken)
      return json({ error: 'Login session missing — dubara admin login karo.' }, 401, cors);
    if (!prompt || !prompt.trim())
      return json({ error: 'Prompt khali hai — kya banana hai batao.' }, 400, cors);
    ...
    const stylePreset =
      {
        festive:
          'festive Indian festival themed food poster, warm string lights, marigold flowers, gold and red accents',
        minimal:
          'clean minimal modern food poster, soft studio lighting, lots of negative space, premium restaurant branding',
        bold: 'bold vibrant street-food poster, high contrast, punchy colors, dynamic diagonal composition',
        discount:
          'eye-catching discount/sale food poster, bright orange and yellow burst shapes, big bold sale typography',
    ...
    try {
      ...
      // Some CF AI versions return raw ArrayBuffer — handle both
      let imageBase64 = null;
      if (result && result.image) {
        imageBase64 = result.image; // already base64 string
      ...
      if (!imageBase64) {
        return json(
          {
            error:
              'AI model se image nahi mili — shayad Cloudflare AI binding sahi set nahi (Worker Settings → Bindings → AI).',
            debug: { resultType: typeof result, resultKeys: result ? Object.keys(result) : null },
          },
          500,
          cors
        );
      ...
      // Give actionable error messages for common CF AI failures
      if (msg.includes('binding') || msg.includes('AI is not defined')) {
        return json(
          {
            error:
              'Workers AI binding missing — Worker Settings → Bindings mein "AI" add karo (type: Workers AI, variable: AI)',
          },
          500,
          cors
        );
      ...
      if (msg.includes('rate limit') || msg.includes('429')) {
        return json(
          {
            error:
              'Cloudflare AI rate limit hit — thodi der baad try karo (free tier: 10k requests/day)',
          },
          429,
          cors
        );
      }
      return json({ error: 'AI generation fail hua: ' + msg }, 500, cors);
    }
  },
};
