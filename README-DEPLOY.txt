...
## ✅ Naya Feature Add Hua (July 2026 — AI Blog Draft Generator)

AI Marketing Agent Worker ab 2 naye endpoints support karta hai:
- `POST /generate-blog-draft` — Search Console ke SEO-opportunity keywords (position 4-25,
  jahan ranking improve karne ka real chance hai) se ek poora blog draft banata hai
  (title, meta description, slug, HTML body). Admin panel mein "AI Blog Draft Generator"
  card se use hota hai.
- `POST /list-blog-drafts` — pehle bane drafts wapas dikhata hai.

⚠️ **Ye sirf DRAFT banate hain, koi file khud publish/deploy nahi hoti.** Draft
Firestore ke `blog_drafts` collection mein `pending_review` status ke saath save hota
hai. Publish karne ke liye: admin panel mein "📋 Copy Full HTML" button dabao, naya
`blog-<slug>.html` file banao (poore site ke existing blog template ke exact structure
mein), Cloudflare Pages pe upload karo, aur `blog.html` + `sitemap.xml` mein manually
link/entry add karo. Ye jaan-bujh kar manual rakha gaya hai — code auto-deploy karna
risky hota, review zaroori hai.

Weekly `runAgent()` flow (Monday cron) ab automatically ek blog draft bhi generate
karta hai, saath mein hafte ki report ke.

`firestore.rules` mein `blog_drafts` collection ke liye admin-only rule add hui hai —
naya rules file deploy karna mat bhoolna (Firebase Console → Firestore → Rules → paste
karo `firestore.rules` ka content).
