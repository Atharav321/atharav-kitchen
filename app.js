/* IMPORTANT: Apna actual Google Maps API key yahan daalo.
   Netlify pe Environment Variable set karo: GMAPS_KEY
   Abhi ke liye placeholder — geocoding ke liye actual key chahiye */
var GMAPS_KEY='AIzaSyD7Vb4zFHfzsI79BbHjZTIi0s8Asxte6rI';
/* SECURITY NOTE: Google Maps key ke liye Firebase Console mein
   HTTP referrer restriction lagao:
   https://console.cloud.google.com/apis/credentials
   Allowed: atharavkitchen.netlify.app/* */
...
function openTrackModal(){document.getElementById('track-modal').classList.add('open');if(!trackMap)loadGoogleMapsAndInit();}
function closeTrackModal(){document.getElementById('track-modal').classList.remove('open');}
function loadGoogleMapsAndInit(){if(window.google&&window.google.maps){initTrackMap();return;}var s=document.createElement('script');s.src='https://maps.googleapis.com/maps/api/js?key='+GMAPS_KEY+'&callback=initTrackMap';s.async=true;document.head.appendChild(s);}
window.initTrackMap=function(){
  ...
  kitchenMarker=new google.maps.Marker({position:{lat:KITCHEN_LAT,lng:KITCHEN_LNG},map:trackMap,title:'Atharav Kitchen',icon:{url:'data:image/svg+xml;charset=UTF-8,'+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 44 44"><circle cx="22" cy="22" r="20" fill="#FF6B00" stroke="#fff" stroke-width="3"/><text x="22" y="28" text-anchor="middle" font-size="18">🏪</text></svg>'),scaledSize:new google.maps.Size(44,44)}});
  deliveryMarker=new google.maps.Marker({position:{lat:delivery_lat,lng:delivery_lng},map:trackMap,title:'Delivery Partner',icon:{url:'data:image/svg+xml;charset=UTF-8,'+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 44 44"><circle cx="22" cy="22" r="20" fill="#1B4332" stroke="#F0CC5A" stroke-width="3"/><text x="22" y="28" text-anchor="middle" font-size="18">🛵</text></svg>'),scaledSize:new google.maps.Size(44,44)}});
  var step=0,steps=60,dlat=(KITCHEN_LAT-delivery_lat)/steps,dlng=(KITCHEN_LNG-delivery_lng)/steps;
  var timer=setInterval(function(){step++;delivery_lat+=dlat;delivery_lng+=dlng;if(deliveryMarker)deliveryMarker.setPosition({lat:delivery_lat,lng:delivery_lng});var mins=Math.round((steps-step)*0.5);var el=document.getElementById('track-status-text');if(el)el.textContent=step>=steps?'✅ Delivery partner arrived!':'🛵 On the way — ETA '+mins+' mins';if(step>=steps)clearInterval(timer);},800);
};
...
// ---- LOADER ----
window.addEventListener('load',function(){setTimeout(function(){var l=document.getElementById('loader');if(l)l.classList.add('hide');},1900);
...
function applyDeliveryDistanceFromCoords(lat,lng){
  ...
  if(d>MAX_DELIVERY_KM){
    ...
    alert('Sorry, we only deliver within 5km');
  }else{
...
function renderMenu(){
  ...
  grid.innerHTML=filtered.map(function(item){
    ...
    var imgHtml=imgSrc?'<img src="'+imgSrc+'" style="width:100%;height:140px;object-fit:cover;" alt="'+esc(item.name)+'" loading="lazy">':'<div style="font-size:2.8rem;height:140px;display:flex;align-items:center;justify-content:center;">'+item.emoji+'</div>';
...
function goStep(n){
  if(n===4){
    ...
    if(!withinDeliveryRadius){alert('Sorry, we only deliver within 5km');return;}
...
function placeOrder(){
  if(!customerLoggedIn()){showToast('Please Login to Order','red');return;}
  if(!deliveryRadiusChecked||!withinDeliveryRadius){alert('Sorry, we only deliver within 5km');return;}
  var name=(document.getElementById('ord-name').value||'').trim();
  var phone=(document.getElementById('ord-phone').value||'').trim();
  var addr=(document.getElementById('ord-address').value||'').trim();
