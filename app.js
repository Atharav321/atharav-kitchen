...
function isGuestOrder(){
  if(akFirebaseReady&&firebase&&firebase.auth&&firebase.auth().currentUser)return false;
  if(currentUser&&currentUser.phone)return false;
  return true;
}

function updateCheckoutLockUI(){
  var loggedIn=(akFirebaseReady&&firebase&&firebase.auth&&firebase.auth().currentUser)||(currentUser&&currentUser.phone);
  ...
  // Place order button — block only if delivery radius not checked/outside range
  ...
  firebase.auth().signInWithPopup(provider)
    ...
    .catch(function(e){
      if(e.code==='auth/popup-blocked'||e.code==='auth/popup-cancelled-by-user'||e.code==='auth/cancelled-popup-request'){
        // Popup blocked (mostly mobile) — redirect fallback
        firebase.auth().signInWithRedirect(provider);
      } else if(e.code==='auth/popup-closed-by-user'){
        // User ne popup band kiya — kuch mat karo
        return;
      } else {
        showToast('Google login failed: '+(e.message||'Try again'),'red');
      }
...
function placeOrder(){
  ...
  if(!deliveryRadiusChecked||!withinDeliveryRadius){showToast('Delivery range check pending ya range se bahar. 📍','red');return;}
...
function showTrackFAB(orderId){
  ...
  // Start real-time Firestore tracking immediately
...
(function(){
  ...
  if(o&&o.status!=='Delivered'&&o.status!=='Cancelled'){
    setTimeout(function(){
      ...
      startOrderTracking(lastId); // start real-time listener
    },2000);
  }
})();
...
/* ------------------------------------------------
   3. LIVE ORDER TRACKING — Firestore real-time listener
      (Firebase se instant update, localStorage fallback)
   ------------------------------------------------ */
var _trackingUnsub=null; // Firestore listener reference

function startOrderTracking(orderId){
  // Clear any previous listener
  if(_trackingUnsub){try{_trackingUnsub();}catch(e){}_trackingUnsub=null;}

  // Show initial status immediately from localStorage
  var orders=lsGet('ak_orders',[]);
  var order=orders.find(function(o){return o.id===orderId;});
  if(order)showOrderStatusBar(order);

  // Firebase real-time listener (primary)
  if(akFirebaseReady){
    try{
      _trackingUnsub=firebase.firestore().collection('orders').doc(orderId)
        .onSnapshot(function(snap){
          if(!snap.exists)return;
          var data=snap.data();
          // Sync to localStorage so offline still works
          var orders2=lsGet('ak_orders',[]);
          var idx=orders2.findIndex(function(o){return o.id===orderId;});
          if(idx>-1){orders2[idx].status=data.status;orders2[idx].riderName=data.riderName||'';}
          lsSet('ak_orders',orders2);
          showOrderStatusBar(data);
          // Stop listening once delivered/cancelled
          ...
          console.warn('Firestore tracking error, falling back to poll:',err);
          _startTrackingPoll(orderId); // fallback
        });
    }catch(e){
      _startTrackingPoll(orderId);
    }
  }else{
    // No Firebase — poll localStorage every 8 seconds
    _startTrackingPoll(orderId);
  }
}

function _startTrackingPoll(orderId){
  var trackInterval=setInterval(function(){
    var orders=lsGet('ak_orders',[]);
    var order=orders.find(function(o){return o.id===orderId;});
    if(!order){clearInterval(trackInterval);return;}
    showOrderStatusBar(order);
    if(order.status==='Delivered'||order.status==='Cancelled')clearInterval(trackInterval);
  },8000);
}
