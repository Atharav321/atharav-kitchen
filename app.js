...
window.addEventListener('akFirebaseReady',function(){
  if(!akFirebaseReady){
    ...
    try{ensureKitchenStatusListener();}catch(e){}
  }
});
...
function ensureKitchenStatusListener(){
  ...
  akKitchenStatusUnsub=subscribeKitchenStatus(function(snap){
      ...
      var notice=document.getElementById('kitchen-closed-notice');
      var badge=document.querySelector('.hero-badge');
      if(notice){notice.style.display=akKitchenOpen?'none':'flex';}
      if(badge){badge.style.display=akKitchenOpen?'':'none';}
      try{updateCheckoutLockUI();}catch(e){}
    },function(err){
      console.warn('[Atharav Kitchen] Kitchen status sync error — falling back to cached/localStorage value.',err);
    });
}

function ensureAnnouncementsListener(){
  if(!akFirebaseReady||!firebase||!firebase.firestore)return;
  subscribeAnnouncements(function(snap){
    var d=snap.exists?snap.data():{};
...
function renderAnnouncementBanners(banners){
  ...
  wrap.innerHTML=banners.map(function(b){
    return '<div style="background:'+(colorMap[b.color]||colorMap.forest)+';color:#fff;padding:0.7rem 1rem;text-align:center;font-weight:700;font-size:0.85rem;">'+esc(b.text)+'</div>';
  }).join('');
}
...
function ensureHeroContentListener(){
  if(!akFirebaseReady||!firebase||!firebase.firestore)return;
  subscribeHeroContent(function(snap){
    var d=snap.exists?snap.data():{};
...
function updateCheckoutLockUI(){
  ...
  if(typeof akKitchenOpen!=='undefined' && akKitchenOpen===false){
    ...
    if(banner){banner.textContent='🔴 Kitchen abhi band hai — ordering temporarily disabled hai.';banner.classList.add('show');}
    return;
  }
...
function loadCustomerProfile(uid){
  ...
  return getCustomerDoc(uid).then(function(snap){
...
function sendRegisterOTP(){
  ...
  var phone=document.getElementById('reg-phone').value.replace(/\D/g,'').replace(/^0+/,'').trim();
...
function sendLoginOTP(){
  // SECURITY: Rate limit
  if(!akRateLimit('sendOTP',3,300000)){showToast('OTP limit reached! 5 minutes baad try karo.','red');return;}
  var phone=document.getElementById('login-phone').value.replace(/\D/g,'').replace(/^0+/,'').trim();
...
function startMenuFirebaseSync(){
  ...
  akMenuUnsub=subscribeMenu(function(snap){
...
function _doRenderMenu(){
  ...
  grid.innerHTML=filtered.map(function(item){
    ...
    return '<div class="mc" data-item-name="'+esc(item.name)+'"><div class="mc-top" onclick="openItemDetail('+item.id+')" style="cursor:pointer;">'+imgHtml+vegIcon+bsTag+heartBtn+'</div>'+
      ...
      '<div class="mc-foot"><span class="mc-price">₹'+item.price+'</span><span class="mc-qty-slot">'+btnHtml+'</span></div></div></div>';
  }).join('');
}
...
function buildMenuCardQtyHtml(item){
  ...
  if(inCart>0){
    return '<div style="display:flex;align-items:center;gap:6px;background:#FF6B00;border-radius:8px;padding:4px 8px;">'+
      '<button onclick="changeQty(\''+safeName+'\','+item.price+',-1,event)" style="background:transparent;border:none;color:#fff;font-size:1rem;cursor:pointer;font-weight:800;line-height:1;padding:0 2px;">−</button>'+
      '<span style="color:#fff;font-weight:800;font-size:0.9rem;min-width:16px;text-align:center;">'+inCart+'</span>'+
      '<button onclick="changeQty(\''+safeName+'\','+item.price+',1,event)" style="background:transparent;border:none;color:#fff;font-size:1rem;cursor:pointer;font-weight:800;line-height:1;padding:0 2px;">+</button>'+
      '</div>';
  }
  return '<button class="mc-add" onclick="addCart(\''+safeName+'\','+item.price+',event)">+ Add</button>';
}
...
function updateMenuCardQtyInPlace(name){
  ...
  if(!card){renderMenu(true);return;}
  var slot=card.querySelector('.mc-qty-slot');
  var item=getMenu().find(function(i){return i.name===name;});
  if(!slot||!item){renderMenu(true);return;}
...
function addFromItemDetail(){
  ...
  updateCartBar();updateMenuCardQtyInPlace(item.name);
...
function addCart(name,price,e){
  ...
  cart[name].qty++;
  updateCartBar();updateMenuCardQtyInPlace(name);
...
function goStep(n){
  ...
  if(n===4){
    ...
    if(phone.replace(/\D/g,'').replace(/^0+/,'').length!==10){showToast('Valid 10-digit phone number daalo!','red');return;}
  }
  currentStep=n;
...
function renderUpsellBox(){
  ...
  box.innerHTML='<div style="font-size:0.72rem;font-weight:800;color:var(--text-light,#A08060);text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;">🔥 Log ye bhi order karte hain</div>'+
    suggestions.map(function(i){
      ...
      var thumbHtml=thumbSrc?'<img src="'+thumbSrc+'" style="width:22px;height:22px;border-radius:6px;object-fit:cover;display:inline-block;vertical-align:middle;margin-right:4px;">':'';
      return '<div style="display:flex;align-items:center;justify-content:space-between;background:#FFF7ED;border:1.5px solid #FFE4C4;border-radius:10px;padding:8px 12px;margin-bottom:6px;">'+
        '<span style="font-size:0.82rem;font-weight:700;color:#2D1A00;">'+thumbHtml+esc(i.name)+' <span style="color:#FF6B00;">₹'+i.price+'</span></span>'+
...
function tapCoupon(code){
  ...
  // Welcome coupon already used by this account — block re-applying it,
  // even though the code itself still technically exists in COUPONS.
  if(currentUser&&code===currentUser.welcomeCode&&currentUser.welcomeCodeUsed){
    res.style.display='block';res.style.background='#FEE2E2';res.style.color='#DC2626';res.style.border='1px solid #FECACA';
    res.textContent='❌ Yeh welcome coupon already use ho chuka hai — sirf ek baar milta hai.';
    appliedCoupon=null;
    return;
  }
...
function placeOrder(){
  ...
  function afterSaved(){
    ...
    summaryHtml+='<div class="success-row" style="font-size:0.8rem;">📱 <span>Confirm hone par <strong>'+esc(name)+'</strong> ko WhatsApp aayega</span></div>';
    ...
    updateCartBar();renderMenu(true);updateWalletUI();
    ...
    setTimeout(function(){
      ...
      msg+='\n📋 *ORDER ITEMS:*\n';
      Object.entries(orderObj.items).forEach(function(e){msg+='• '+e[0]+' × '+e[1].qty+' = Rs.'+(e[1].qty*e[1].price)+'\n';});
...
function loadWalletFromFirebase(){
  ...
  if(!ru)return;
  var uid=ru.uid;
  getWalletDoc(uid).then(function(snap){
    if(snap.exists){
...
function sendWhatsAppBill(order){
  ...
  if(Array.isArray(items)){
    items.forEach(function(it){msg+='• '+it.name+' × '+it.qty+' = Rs.'+(it.qty*it.price)+'\n';});
  }else{
    Object.entries(items).forEach(function(e){msg+='• '+e[0]+' × '+e[1].qty+' = Rs.'+(e[1].qty*e[1].price)+'\n';});
  }
  msg+='\n💰 *BILL DETAILS:*\n';
...
function quickFeedback(val){
  ...
  var fb={id:Date.now(),name:(currentUser&&currentUser.name)||'Guest',customerId:(currentUser&&(currentUser.id||currentUser.phone))||'guest',quick:val,rating:val};
  saveFeedback(fb);
  closeFeedbackPopup();
  if(val>=4){
...
function openTrackModal(){
  ...
  if(akFirebaseReady){
    ...
    _trackingUnsubscribe=subscribeOrder(lastOrderId,function(snap){
...
function renderTrackingUI(order){
  ...
  if(order.deliveredBy&&(status==='Out for Delivery'||status==='Preparing')){
    ...
    if(riderPhone)riderPhone.textContent=order.riderPhone?'+91 '+order.riderPhone:'Rider assigned';
    if(riderCall){
      if(order.riderPhone)riderCall.href='tel:+91'+String(order.riderPhone).replace(/[^0-9]/g,'');
...
(function(){
  ...
  if(akFirebaseReady){
    getOrderOnce(lastId).then(function(snap){
      if(!snap.exists){
    ...
    }).catch(function(){/* offline — silently skip, will re-check next load */});
  }else{
...
document.addEventListener('keydown',function(e){
  if(e.key==='Escape'){
    ...
    if(document.getElementById('track-modal')&&document.getElementById('track-modal').style.display==='flex'&&typeof closeTrackModal==='function')closeTrackModal();
  }
});
...
function loadKitchenGallery(){
  ...
  getKitchenGallerySettings().then(function(doc){
...
function submitAutoRating(orderJson){
  ...
  var order=JSON.parse(orderJson);
  var fb={id:Date.now(),name:(currentUser&&currentUser.name)||'Guest',customerId:currentUser?(currentUser.id||currentUser.phone):'guest',orderId:(order.id||order.orderId),rating:_autoRatingVal,autoPrompt:true};
  saveFeedback(fb);
...
function openReferralModal(){
  ...
  if(ru&&akFirebaseReady){
    getReferralStats(ru.uid).then(function(snap){
...
SPIN_PRIZES.forEach(function(p){
  ...
  if(!COUPONS[p.code]){
    ...
    COUPONS[p.code]={type:p.type,value:p.value,min:0,maxDisc:maxDisc,label:p.label+' (Spin Prize)'};
  }
});
...
function showSpinResult(prize){
  ...
  if(prize.code){
    ...
    if(akFirebaseReady&&firebase.auth().currentUser){
      logSpinWin({uid:firebase.auth().currentUser.uid,prize:prize.label,code:prize.code,date:new Date().toISOString()}).catch(function(){});
    }
