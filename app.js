...
function sendLoginOTP(){
  // SECURITY: Rate limit — same bucket as register OTP (boundary layer 7)
  if(!akRateLimit('sendOTP',3,300000)){showToast('OTP limit reached! 5 minutes baad try karo.','red');return;}
  ...
  try{
    // FIX: Clear old recaptcha completely + reset DOM node
    if(_loginRecaptcha){try{_loginRecaptcha.clear();}catch(e){}_loginRecaptcha=null;}
    var rcElL=document.getElementById('recaptcha-login');if(rcElL)rcElL.innerHTML='';
    _loginRecaptcha=new firebase.auth.RecaptchaVerifier('recaptcha-login',{
      'size':'invisible','callback':function(){},
      'expired-callback':function(){if(_loginRecaptcha){try{_loginRecaptcha.clear();}catch(e){}_loginRecaptcha=null;}}
    });
    _loginRecaptcha.render().then(function(){
      ...
      .catch(function(e){
        ...
        var rcElL2=document.getElementById('recaptcha-login');if(rcElL2)rcElL2.innerHTML='';
        var msg='OTP bhejne mein error.';
        if(e.code==='auth/invalid-phone-number')msg='Phone number galat hai. 10 digit daalo.';
        else if(e.code==='auth/too-many-requests')msg='Bahut zyada attempts. 10 minute baad try karo.';
        else if(e.code==='auth/app-not-authorized')msg='Firebase Phone Auth enable karo: Console → Auth → Phone.';
...
function placeOrder(){
  ...
  // SECURITY: Rate limit — max 3 order attempts per 60 seconds (boundary layer 5)
  ...
  // SECURITY: Strict input validation (boundary layer 6)
  if(!akValidateName(name)){showToast('Invalid name — only letters, numbers, spaces allowed!','red');return;}
  if(!akValidatePhone(phone.replace(/\D/g,''))){showToast('Enter valid 10-digit Indian mobile number!','red');return;}
  if(!akValidateAddress(addr)){showToast('Enter a valid delivery address (min 6 chars)!','red');return;}
  if(note.length>200){showToast('Note too long (max 200 chars)','red');return;}
  var cleanPhone=phone.replace(/\D/g,'');
  // SECURITY: Strip ALL special chars from user fields before storage
  name=name.replace(/[<>'"&]/g,'');
  addr=addr.replace(/[<>'"&]/g,'');
  note=note.replace(/[<>'"&]/g,'');
  ...
  function afterSaved(){
    ...
    summaryHtml+='<div class="success-row" style="background:#FEF9C3;border:1.5px solid #FDE68A;border-radius:8px;padding:8px 12px;margin-top:8px;font-size:0.85rem;">⏳ <span><strong>Order Received!</strong> Owner se confirm hone ka wait karo.</span></div>';
    summaryHtml+='<div class="success-row" style="font-size:0.8rem;">📱 <span>Confirm hone par <strong>'+phone+'</strong> pe WhatsApp aayega</span></div>';
    ...
    var _st=document.querySelector('#order-success .success-title');if(_st)_st.textContent='📋 Order Received!';
    var _ss=document.querySelector('#order-success .success-subtitle');if(_ss)_ss.textContent='Owner aapka order dekh raha hai — confirmation ka wait karo ⏳';
...
placeOrder=function(){
  ...
  function afterSaved(){
    ...
    // Award points (only after successful save — but before owner confirms, as gesture)
    awardPoints(orderObj);
    // Deduct wallet points
    if(walletDisc>0)deductWalletPoints(walletDisc);
    // NOTE: sendWhatsAppBill REMOVED from here.
    // Customer ko confirmation tab milega jab owner admin panel mein Accept karega.
    ...
    summaryHtml+='<div class="success-row">💰 <span>Total: <strong>₹'+bill.total+'</strong></span></div>';
    // ✅ CORRECT FLOW: Customer ko PENDING status dikhao, CONFIRMED nahi
    summaryHtml+='<div class="success-row" style="background:#FEF9C3;border:1.5px solid #FDE68A;border-radius:8px;padding:8px 12px;margin-top:8px;font-size:0.85rem;">⏳ <span><strong>Order Received!</strong> Owner se confirm hone ka wait karo.</span></div>';
    summaryHtml+='<div class="success-row" style="font-size:0.8rem;">📱 <span>Confirm hone par <strong>'+phone+'</strong> pe WhatsApp aayega</span></div>';
    summaryHtml+='<div class="success-row" style="font-size:0.75rem;color:#A08060;border-top:1px dashed #EEE;padding-top:8px;margin-top:4px;">Order ID: <strong>'+orderId+'</strong> — Screenshot karke rakhein</div>';
    document.getElementById('success-summary').innerHTML=summaryHtml;
    // Update success box title to show PENDING not CONFIRMED
...
checkAuthOnLoad();
checkUserDeliveryRadius();
// FIX 4: Restore tracking FAB if there's a recent active order
(function(){
  var lastId=lsGet('ak_last_order_id',null);
  if(lastId){
    var orders=lsGet('ak_orders',[]);
    var o=orders.find(function(x){return x.id===lastId;});
    if(o&&o.status&&o.status!=='Delivered'&&o.status!=='Cancelled'){
      setTimeout(function(){
        var fab=document.getElementById('track-order-fab');
        if(fab)fab.style.display='block';
...
/* ------------------------------------------------
   3. ADMIN QUICK STATUS — customer side order tracking
      (polls localStorage every 10s for status update)
   ------------------------------------------------ */
function startOrderTracking(orderId){
  var trackInterval=setInterval(function(){
    var orders=lsGet('ak_orders',[]);
    var order=orders.find(function(o){return o.id===orderId;});
    if(!order){clearInterval(trackInterval);return;}
